    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center" onClick={onClose} role="dialog" aria-modal="true">
      <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-6 m-4" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-gray-100">Cài đặt API Keys</h2>
            <button onClick={onClose} className="text-gray-400 hover:text-white text-2xl leading-none">&times;</button>
        </div>
        
        <div className="mb-6">
          <p className="text-gray-400 mb-4 text-sm">
              Dán các Gemini API key của bạn, mỗi key một dòng. Hệ thống sẽ tự động chuyển key nếu hết hạn ngạch.
          </p>
          <textarea
            value={keys}
            ...
            placeholder="AIzaSy...
AIzaSy..."
            className="w-full h-32 p-3 bg-gray-900 border border-gray-700 rounded-md focus:ring-2 focus:ring-red-500 focus:outline-none resize-none font-mono text-sm"
        ...
        <div className="mt-8 flex justify-end space-x-2">
            ...
            <button onClick={handleSave} className="px-5 py-2 bg-red-600 text-white font-bold rounded-md hover:bg-red-700 transition-colors">Lưu Keys</button>
        </div>
      </div>
    </div>
  );
